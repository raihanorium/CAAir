 ...
 */
class CancelFlightActionService extends BaseService implements ActionServiceIntf {
    ...
    /**
     * While canceling an Flight object this method is called first.
     * In this method check all preconditions before creating a new Flight object (i.e required params existence, validity of Flight object property)
    ...
    public Map executePreCondition(Map params) {
        try {
            ...
            // @todo check server side validation process (validity of Flight object property i.e. departureTime)
            // @todo check other logic if you might need before cancel Flight
            return params
        } catch (Exception ex) {
     ...
     * Cancel all ticket of the flight
     * Refund money whose are payment of paid ticket.
     * Cancel Flight object
    ...
    public Map execute(Map result) {
        try {
            ...
            // @todo cancel all ticket of the flight, such as ticketInfoService.cancelAllTicket(flight)
            // @todo refund money to customer. who paid for ticket, such as paymentInfoService.refundMoney(flight)
            // @todo update Flight object, such as flightService.update(flight)
            // @todo put updated Flight object in result for future reference.
            return result
        } catch (Exception ex) {
            log.error(ex.getMessage())
            throw new RuntimeException(ex)
     ...
     * Sent email notification to inform customer that fight is canceled
     * sent email notification to inform customer (only those who paid already) that their payment is refunded
     * Put this method in transactional(readOnly) block if only DB read operation is needed or transactional(read/write) if both read/write is needed.
     *
    ...
    public Map executePostCondition(Map result) {
        try {
            ...
            // this email should be generated from template
            // @todo sent email notification to inform customer (only those who paid already) that their payment is refunded
